# 工位素材

| 文件 | 用途 |
| --- | --- |
| `workers/working.png` | 低头工作 |
| `workers/cheering.png` | 举手欢呼 |
| `workers/sleeping.png` | 歪头睡眠 |
| `preview.png` / `preview.gif` | 静态 / 动态预览，使用虚构任务 |

三张工位图为本项目制作的 AI 辅助生成素材，经过本地透明背景处理。应用直接打包并加载这些 PNG；运行时不会生成图片或访问图像服务。

生产素材均为 340 × 360 像素的 RGBA PNG，等比绘制在正方形工位中。运行时只为工帽选择六种配色之一，并保持同一任务颜色固定。小人与睡眠 Z 分层播放轻量动画，电脑和桌面固定，透明背景保留。`preview.png` 和 `preview.gif` 分别由 `--render-preview` 和 `--render-animation` 生成。

本机的 `concepts/` 为开发草图，已被 Git 忽略，不属于源码分发包。素材许可随项目最终选择的许可另行明确；当前没有单独授权声明。
